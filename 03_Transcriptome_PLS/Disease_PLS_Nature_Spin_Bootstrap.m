% =========================================================================
% Transcriptomic Association: Disease Effect using Partial Least Squares (PLS)
% Regression with Spatial Permutation Testing (Spin-test)
% =========================================================================
clear; clc; close all;
...
map_path    = './Data_Sample/Disease_Effect_for_PLS_Tonly.csv';
coords_path = './Data_Sample/Schaefer400_Centroid_RAS.csv';
output_dir  = './Output_PLS_Results/';

if ~exist(output_dir, 'dir'); mkdir(output_dir); end

fprintf('--- Loading Multimodal & Transcriptomic Data (Disease Effect) ---\n');
...
Y = Y_raw(:, end); % Extract the neuroimaging effect size map (T-values)
...
% -------------------------------------------------------------------------
n_spin = 1000;
% Matrix to store null distributions for all 10 components
...
gene_Z = gene_weight ./ gene_std; % Calculate Bootstrap Z-scores
...
writetable(res_sorted, fullfile(output_dir, 'Disease_PLS1_Full_Results.csv'));
num_top = round(0.05 * n_gene);
writetable(res_sorted(1:num_top, :), fullfile(output_dir, 'Disease_Top_5_Percent_Genes.csv'));
...
subplot(1, 2, 2);
r_obs_pls1 = corr(XS(:, 1), Y_scaled); % True observed correlation
